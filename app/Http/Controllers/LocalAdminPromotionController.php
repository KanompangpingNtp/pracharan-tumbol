...
class LocalAdminPromotionController extends Controller
{
    public function LocalAdminPromotionPage()
    {
        $postTypes = PostType::all();

        $postTypeId = $postTypes->firstWhere('type_name', 'กรมส่งเสริมการปกครองท้องถิ่น')->id;
        $postDetails = PostDetail::with('postType', 'pdfs')
            ->where('post_type_id', $postTypeId)
            ->orderBy('created_at', 'desc')
            ->get();

        return view('admin.post.local-admin-promotion.local-admin-promotion', compact('postDetails', 'postTypes'));
    }

    public function LocalAdminPromotionCreate(Request $request)
    {
        $request->validate([
    ...
    public function LocalAdminPromotionUpdate(Request $request, $id)
    {
        $request->validate([
            'date' => 'nullable|date',
            'title_name' => 'nullable|string',
    ...
    public function LocalAdminPromotionDelete($id)
    {
