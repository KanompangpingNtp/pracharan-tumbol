...
<h2 class="text-center">แผนดำเนินการประจำปี</h2><br>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel"> สร้างแผนดำเนิน</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{route('AnnualActionPlanCreate')}}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
                        <input type="hidden" name="perf_results_type" value="{{ $perfResultsType->firstWhere('type_name', 'แผนดำเนินการประจำปี')->id }}">
...
<table class="table table-bordered" id="data_table">
    ...
    <tbody>
        ...
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>
                <a href="{{ route('AnnualActionPlanShowDertails', $detail->id) }}">
                    {{ $detail->detail_name }}
                </a>
            </td>
            ...
            <td class="text-center">
                ...
                <div class="d-inline-block">
                    <form action="{{ route('AnnualActionPlanDelete', $detail->id) }}" method="POST" onsubmit="return confirm('คุณต้องการลบข้อมูลนี้?');">
                        @csrf
                        @method('DELETE')
...
<div class="modal fade" id="editModal{{ $detail->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $detail->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('AnnualActionPlanUpdate', $detail->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
