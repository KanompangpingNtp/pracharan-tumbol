...
<h2 class="text-center">นโยบายการบริหาร/เจตจำนงสุจริต <span class="text-primary">{{$listDetail->list_details_name}}</span> </h2><br>

<div class="card">
    <div class="card-body">
        @if($listDetail->details !== null)
        <form action="{{ route('ManagementPolicyDetailsDelete', $listDetail->id) }}" method="POST" onsubmit="return confirm('คุณต้องการลบข้อมูลนี้หรือไม่?');">
            @csrf
            @method('DELETE')
...
<form action="{{ route('ManagementPolicyDertailsCreate', $listDetail->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
