...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{route('HRPolicyDertailsCreate', $PerfResultsDetail->id)}}" method="POST">
                @csrf
...
<table class="table table-bordered" id="data_table">
    ...
    <tbody>
        ...
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>
                <a href="{{ route('HRPolicyShowDertailResults', $detail->id) }}">
                    {{ $detail->detail_name }}
                </a>
            </td>
            ...
            <td class="text-center">
                ...
                <div class="d-inline-block">
                    <form action="{{ route('HRPolicyDertailsDelete', $detail->id) }}" method="POST" onsubmit="return confirm('คุณต้องการลบข้อมูลนี้?');">
                        @csrf
                        @method('DELETE')
...
<div class="modal fade" id="editModal{{ $detail->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $detail->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('HRPolicyDertailsUpdate', $detail->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
