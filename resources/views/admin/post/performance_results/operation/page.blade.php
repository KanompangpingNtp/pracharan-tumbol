...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{route('OperationCreate')}}" method="POST" enctype="multipart/form-data">
                @csrf
...
<table class="table table-bordered text-center" id="data_table">
    ...
    <tbody>
        @foreach($PerfSingleTopicFiles as $index => $file)
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>{{ basename($file->file_path) }}</td>
            <td>{{ strtoupper($file->file_type) }}</td>
            <td class="text-center">
                <a href="{{ asset('storage/' . $file->file_path) }}" target="_blank" class="btn btn-primary">
                    <i class="bi bi-file-earmark-pdf"></i>
                </a>

                <form action="" method="POST" class="d-inline" onsubmit="return confirm('คุณต้องการลบไฟล์นี้หรือไม่?');">
                    @csrf
                    @method('DELETE')
