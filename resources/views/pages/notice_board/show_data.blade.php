...
<div class="bg py-5">
    <div class="container py-5 custom-gradient-shadow">
        <div class=" d-flex flex-column justify-content-center p-5">
            <div class="fs-1 fw-bold mb-4 text-center" style="color: #77b329;">ป้ายประกาศ</div>

            <div class="row">
                @foreach ($noticeBoard as $index => $post)
                @php
                // กำหนดคลาสพื้นหลังสลับสี
                $cardBackgroundClass = ($index % 2 == 0) ? 'bg-blue-card-section-6' : 'bg-pink-card-section-6';
                @endphp
                <div class="col-lg-6 p-2">
                    <a href="{{ route('NoticeBoardShowDetails', $post->id) }}" class="text-decoration-none">
            ...
            @if($noticeBoard && $noticeBoard->count() > 0)
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center mt-5">
                    <!-- Previous button -->
                    <li class="page-item {{ $noticeBoard->onFirstPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $noticeBoard->previousPageUrl() }}">ก่อนหน้า</a>
                    </li>

                    <!-- Page number buttons -->
                    @foreach ($noticeBoard->getUrlRange(1, $noticeBoard->lastPage()) as $page => $url)
                    <li class="page-item {{ $page == $noticeBoard->currentPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                    </li>
                    @endforeach

                    <!-- Next button -->
                    <li class="page-item {{ !$noticeBoard->hasMorePages() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $noticeBoard->nextPageUrl() }}">ต่อไป</a>
                    </li>
                </ul>
            </nav>
            @endif
