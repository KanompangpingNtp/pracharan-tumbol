...
<div class="bg py-5">
    <div class="container py-5 custom-gradient-shadow">
        <div class=" d-flex flex-column justify-content-center p-5">
            <div class="fs-1 fw-bold mb-4 text-center" style="color: #77b329;">ที่นี่ตำบลพระอาจารย์</div>

            <div class="row">
                @foreach ($RecommendedPlaces as $index => $post)
                @php
                // กำหนดคลาสพื้นหลังสลับสี
                $cardBackgroundClass = ($index % 2 == 0) ? 'bg-blue-card-section-6' : 'bg-pink-card-section-6';
                @endphp
                <div class="col-lg-6 p-2">
                    <a href="{{ route('RecommendedPlacesShowDetails', $post->id) }}" class="text-decoration-none">
            ...
            @if($RecommendedPlaces && $RecommendedPlaces->count() > 0)
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center mt-5">
                    <!-- Previous button -->
                    <li class="page-item {{ $RecommendedPlaces->onFirstPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $RecommendedPlaces->previousPageUrl() }}">ก่อนหน้า</a>
                    </li>

                    <!-- Page number buttons -->
                    @foreach ($RecommendedPlaces->getUrlRange(1, $RecommendedPlaces->lastPage()) as $page => $url)
                    <li class="page-item {{ $page == $RecommendedPlaces->currentPage() ? 'active' : '' }}">
                        <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                    </li>
                    @endforeach

                    <!-- Next button -->
                    <li class="page-item {{ !$RecommendedPlaces->hasMorePages() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $RecommendedPlaces->nextPageUrl() }}">ต่อไป</a>
                    </li>
                </ul>
            </nav>
            @endif
